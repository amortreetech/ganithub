...
router.post('/register', [
    ...
    body('role').isIn(['student', 'tutor', 'parent']).withMessage('Valid role is required')
], async (req, res) => {
    try {
        ...
        const user = await findOne('users', { id: userId }, 'id, first_name, last_name, email, role, phone, date_of_birth, created_at');
...
router.get('/me', async (req, res) => {
    try {
        ...
        // Get user
        const user = await findOne('users', { id: decoded.id }, 'id, first_name, last_name, email, role, phone, date_of_birth, created_at, updated_at');
        if (!user) {
            return res.status(404).json({
                success: false,
                message: 'User not found'
...
router.put('/profile', [
    ...
    try {
        ...
        // Get updated user
        const updatedUser = await findOne('users', { id: decoded.id }, 'id, first_name, last_name, email, role, phone, date_of_birth, created_at, updated_at');
